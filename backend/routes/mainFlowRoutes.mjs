...
router.post("/flow", async (req, res) => {
  try {
    ...
    const idpResponse = await axios.post("http://13.232.45.218:8000/api/idp/extract", {
    ...
    await axios.post("http://13.232.45.218:5050/convert_to_structure", {extractedData: extractedData});

    // 4️⃣ Call FastAPI → rule engine
    const ruleEngineResponse = await axios.post("http://13.232.45.218:5050/rule_engine", {
        request_id: requestId,
        proposalNumber: jsonData.proposal_number
    });

    // // 5️⃣ Save rule engine output in your Node `ruleEngineRoutes`
    // await axios.post("http://13.232.45.218:8000/api/ruleEngineRoutes/save-rule-engine-result", {
