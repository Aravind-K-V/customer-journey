...
const RightSectionLogin = () => {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post("http://13.232.45.218:8000/api/auth/login", {
        email,
        password,
      });
      ...
      if (response.data.message === 'Login successful') {
        ...
        const statusResponse = await axios.get(`http://13.232.45.218:8000/api/auth/underwriting/status/${email}`);
        ...
        switch (status) {
          ...
          case "Rules Applied": {
            ...
            fetch(`http://13.232.45.218:8000/api/ruleEngineRoutes/rule-check?email=${encodeURIComponent(email)}`)
              .then(res => res.json())
              .then(data => {
